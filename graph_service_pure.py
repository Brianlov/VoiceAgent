...
class GraphRAGProcessor(FrameProcessor):
    ...
    def __init__(self, neo4j_uri: str, neo4j_user: str, neo4j_password: str, neo4j_db: str, knowledge_base: List[str] = None):
        ...
        print("🔧 GraphRAG [PURE]: Loading Embedding Model (all-mpnet-base-v2)...")
    ...
    async def _initialize_graph(self):
        ...
        print(f"🧠 GraphRAG [PURE]: Verifying Neo4j Connection at {self.neo4j_uri}...")
        try:
            await self.driver.verify_connectivity()
            print(f"✅ GraphRAG [PURE]: Connected to Neo4j successfully.")
            self._initialized = True
        except Exception as e:
            print(f"❌ GraphRAG [PURE]: Failed to connect to Neo4j: {e}")

    async def retrieve(self, query: str, as_list: bool = False):
        print(f"🚨 RETRIEVE [PURE GraphRAG] CALLED with: '{query}'")
        ...
        # 2. PURE GraphRAG Retrieval
        # Restricts all retrieved context chunks strictly to graph traversals starting from matched entities.
        cypher = """
        ...
        try:
            async with self.driver.session(database=self.neo4j_db) as session:
                print(f"🔍 GraphRAG [PURE]: Executing Cypher traversal query...")
                result = await session.run(cypher, query_vector=query_vector)
                records = await result.data()
                print(f"📦 GraphRAG [PURE]: Retrieval returned {len(records)} candidate traversed chunks")
                ...
                if records:
                    ...
                    print(f"❌ GraphRAG [PURE]: No entities matched query vector.")

        except Exception as e:
            print(f"⚠️ GraphRAG [PURE] Retrieval error: {e}")
            
        if not context_parts:
            print(f"❌ GraphRAG [PURE]: No context found for query='{query}'")
            return [] if as_list else ""
        ...
        final_context = "\n".join(context_parts)
        print(f"✅ GraphRAG [PURE]: Retrieved Context Content:\n{'-'*40}\n{final_context}\n{'-'*40}")
        return final_context[:12000]

    async def _flush_stop_frame(self, direction):
        if self.saved_stop_frame:
            print("🚦 GraphRAG [PURE]: Flushing buffered Stop Frame")
