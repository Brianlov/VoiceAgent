...
class GraphRAGProcessorV4(FrameProcessor):
    LAST_QUERY = ""
    LAST_CONTEXT = ""
    ...
    def __init__(self, neo4j_uri: str, neo4j_user: str, neo4j_password: str, neo4j_db: str, knowledge_base: List[str] = None):
        ...
        print("🔧 GraphRAG [PURE v2]: Loading Embedding Model (all-mpnet-base-v2)...")
    ...
    async def _initialize_graph(self):
        ...
        print(f"🧠 GraphRAG [PURE v2]: Verifying Neo4j Connection at {self.neo4j_uri}...")
        try:
            await self.driver.verify_connectivity()
            print(f"✅ GraphRAG [PURE v2]: Connected to Neo4j successfully.")
            self._initialized = True
        except Exception as e:
            print(f"❌ GraphRAG [PURE v2]: Failed to connect to Neo4j: {e}")

    async def retrieve(self, query: str, as_list: bool = False):
        print(f"🚨 RETRIEVE [PURE GraphRAG v2] CALLED with: '{query}'")
        ...
        if not clean_query:
            clean_query = "placeholder"
            
        # 2. 3-Hop Cypher Query with Hybrid Search
        cypher = """
        // Phase 1a: Vector Search
        CALL db.index.vector.queryNodes('entity_vector_index', 3, $query_vector) 
        YIELD node AS vector_node, score AS vector_score
        WITH collect(vector_node) AS v_nodes
        
        // Phase 1b: Full-Text Keyword Search
        CALL db.index.fulltext.queryNodes('entity_name_index', $clean_query)
        YIELD node AS keyword_node, score AS keyword_score
        WITH v_nodes, collect(keyword_node)[0..3] AS k_nodes
        
        // Phase 1c: Merge
        UNWIND (v_nodes + k_nodes) AS e
        WITH DISTINCT e
        WITH e, 1.0 AS entity_score,
             coalesce(e.name, e.id) + ": " + coalesce(e.description, "No description available") AS entity_desc
             
        // Phase 2: 1-hop relationships (limit to 8 neighbors per entity)
        ...
        // Phase 3: 2-hop relationships (limit to 6 neighbors per 1-hop neighbor)
        ...
        WITH e, entity_score, entity_desc, neighbors1, facts_1hop,
             collect(DISTINCT neighbor2)[0..6] AS neighbors2,
             collect(DISTINCT fact_2hop) AS facts_2hop
             
        // Phase 4: 3-hop relationships (limit to 4 neighbors per 2-hop neighbor)
        UNWIND (case when size(neighbors2) > 0 then neighbors2 else [null] end) AS n2
        OPTIONAL MATCH (n2)-[r3]-(neighbor3:Entity)
        WHERE type(r3) <> "MENTIONED_IN" AND neighbor3 <> e AND NOT neighbor3 IN neighbors1
        WITH e, entity_score, entity_desc, neighbors1, neighbors2, facts_1hop, facts_2hop, neighbor3, r3,
             coalesce(startNode(r3).name, startNode(r3).id) + " " + type(r3) + " " + coalesce(endNode(r3).name, endNode(r3).id) AS fact_3hop
             
        WITH e, entity_score, entity_desc, neighbors1, neighbors2, facts_1hop, facts_2hop,
             collect(DISTINCT neighbor3)[0..4] AS neighbors3,
             collect(DISTINCT fact_3hop) AS facts_3hop
             
        WITH e, entity_score, entity_desc,
             neighbors1 + neighbors2 + coalesce(neighbors3, []) AS neighbors,
             facts_1hop + facts_2hop + coalesce(facts_3hop, []) AS entity_facts
             
        // Phase 5: Get context chunks linked to all matched entities
        ...
        try:
            async with self.driver.session(database=self.neo4j_db) as session:
                print(f"🔍 GraphRAG [PURE v4]: Executing Hybrid 3-hop Cypher traversal query...")
                result = await session.run(cypher, query_vector=query_vector, clean_query=clean_query)
                records = await result.data()
                print(f"📦 GraphRAG [PURE v4]: Retrieval returned {len(records)} candidate traversed chunks")

                if records:
                    candidate_chunks = []
                    seen_texts = set()
                    
                    # 3. Candidate Ranking in Python
                    for r in records:
                    ...
                    top_candidates = candidate_chunks[:15]
                    ...
                    if unique_chunks:
                        ...
                        context_parts.extend(unique_chunks[:15])
                        context_parts.append("") 
                        raw_chunks.extend(unique_chunks[:15])

                    if unique_facts:
                        context_parts.append("[GRAPH RELATIONSHIPS]")
                        context_parts.extend(list(unique_facts)[:40])
                        context_parts.append("") 
                        raw_chunks.extend(list(unique_facts)[:40])
                    ...
                    print(f"❌ GraphRAG [PURE v2]: No entities matched query vector.")

        except Exception as e:
            print(f"⚠️ GraphRAG [PURE v2] Retrieval error: {e}")
            
        if not context_parts:
            print(f"❌ GraphRAG [PURE v2]: No context found for query='{query}'")
            return [] if as_list else ""
            
        if as_list:
            return raw_chunks[:21]
            
        final_context = "\n".join(context_parts)
        print(f"✅ GraphRAG [PURE v2]: Retrieved Context Content:\n{'-'*40}\n{final_context}\n{'-'*40}")
        return final_context[:16000]

    async def _flush_stop_frame(self, direction):
        if self.saved_stop_frame:
            print("🚦 GraphRAG [PURE v2]: Flushing buffered Stop Frame")
